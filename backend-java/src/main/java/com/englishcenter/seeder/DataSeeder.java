...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        // ─── Users ─────────────────────────────────
        User admin = userRepository.save(User.builder().username("admin").password(hashedPassword).fullName("Nguyễn Văn Admin").email("admin@englishcenter.vn").phone("0901234567").role(Role.admin).status(UserStatus.active).build());
